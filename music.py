...
class Music(commands.Cog):
    ...
    async def _join(self, ctx: commands.Context):
        ...
        em = discord.Embed(description=f":zzz: Joined in {destination}", color = 0x36393E)
    ...
    async def _summon(self, ctx: commands.Context, *, channel: discord.VoiceChannel = None):
        ...
        if ctx.voice_state.voice:
            await ctx.voice_state.voice.move_to(destination)
            em = discord.Embed(description=f"Summoned in {destination}", color = 0x36393E)
            em.set_footer(text=f"Requested by {ctx.author.name}")
            await ctx.send(embed=em)
    ...
    async def _leave(self, ctx: commands.Context):
        ...
        em = discord.Embed(description=f"Disconnected from {dest}", color = 0x36393E)
        em.set_footer(text=f"Requested by {ctx.author.name}")            
        await ctx.send(embed=em)
...
# Search whatever u want on youtube!
    ...
    async def syt(self, ctx, *, search):
        ...
        em = discord.Embed(title = ":bulb: **Search Result**", description = "http://youtube.com/watch?v=" + search_content[0], color = 0x36393E)
        em.set_thumbnail(url="https://images-ext-1.discordapp.net/external/Mtpk5n8WXZpkYZlaiSotclUDwpgzmfvOCFN1-mpZMkw/https/cdn.upload.systems/uploads/2EboKc45.png")
    ...
    async def _volume(self, ctx: commands.Context, *, volume:int):
        ...
        em = discord.Embed(title=f"Volume set at the **`{volume}%`**", color = 0x36393E)
    ...
    async def _stop(self, ctx):
        ...
        em = discord.Embed(title=f":zzz: Alright, i'll stop the current song.", color = 0x36393E)
    ...
    async def _queue(self, ctx: commands.Context, *, page: int = 1):
        ...
        embed = (discord.Embed(color = 0x36393E, description='**{} Tracks:**\n\n{}'.format(len(ctx.voice_state.songs), queue))
